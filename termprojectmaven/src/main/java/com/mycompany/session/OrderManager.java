...
//        order.setConfirmationNumber(i);
    ...
    private void addOrderedItems(CustomerOrder order, ShoppingCart cart) {
        ...
        for (ShoppingCartItem scItem : items) {
            ...
            //set up primary key
            CustomerOrderHasProductPK orderedProductPK = new CustomerOrderHasProductPK();
            orderedProductPK.setCustomerOrderId(order.getId());
            orderedProductPK.setProductId(productId);

            //create ordered item using PK object
            CustomerOrderHasProduct orderedItem = new CustomerOrderHasProduct(orderedProductPK);
        ...
        List<CustomerOrderHasProduct> orderedProducts = customerOrderHasProductFacade.findByOrderId(orderId);

        // get product details for ordered items
        List<Product> products = new ArrayList<Product>();

        for (CustomerOrderHasProduct op : orderedProducts) {
            Product p = (Product) productFacade.find(op.getCustomerOrderHasProductPK().getProductId());
            products.add(p);
        }
